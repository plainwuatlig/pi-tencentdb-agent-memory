...
- **Knowledge-map injection, once per session (optional)** — set `TDAI_INJECT_MAP="<wiki_id>:<page ref>"` and the named wiki page is injected first, before the persona, wrapped in `<tdai-knowledge-map>`. The point is to advertise what the knowledge base *contains*, not merely that it exists: an agent that cannot predict a wiki hit pattern-matches "locate X" to a filesystem sweep. The page is an ordinary wiki page your team maintains; this extension carries no team content. Re-armed after `/compact` and retried on the next turn after an outage, independently of the persona push.
- **Visible delivery** — all injections land as an ordinary session message (`customType: "tdai-memory-inject"`, `display: true`), never a hidden splice — every injection is a reviewable row in the transcript.
- **Query-first routing guidance** — the tool descriptions tell the model to search tdai memory and the wiki *before* hunting the filesystem for a repo, host, config, or deploy recipe, so a memory miss, not a `find ~` sweep, is the first move.
...
# or from git:
pi install git:github.com/plainwuatlig/pi-tencentdb-agent-memory@v0.2.0
...
| `TDAI_INJECT_MAP` | no | `<wiki_id>:<page ref>` of a knowledge-map page to inject once per session (e.g. `wiki-abc123:wiki/index/knowledge-map.md`); unset = no map injection |
